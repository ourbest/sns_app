...
class RequestCalling:
    ...
    def pull_connection(self) -> CallingList or None:
    ...
    def update(self, status: int, qq_number=None) -> CallingList or None:
        connection = self.pull_connection()
        if not connection or connection.status + 1 != status:
            return

        role = self.what_role(connection)
        if status == 1 and role == self.roles[1]:
            connection.status += 1
            connection.save()
        elif status == 2 and role == self.roles[1] and qq_number:
            qq = model_manager.get_qq(qq_number)
            if qq:
                connection.status += 1
                connection.called_qq = qq
                connection.save()
        elif status == 3 and role == self.roles[0]:
            connection.status += 1
            connection.save()
        elif status == 4 and role == self.roles[0]:
            connection.status += 1
            connection.save()
        elif status == 5 and role == self.roles[1]:
            connection.status += 1
            connection.save()
        elif status == 6 and role == self.roles[0]:
            connection.status += 1
